...
**Ротация `SECRET_KEY` не автоматизирована**: смена значения делает старые
шифртексты нечитаемыми (перешифрование не встроено). Менять ключ — только с
планом перевыпуска всех зависимых секретов. Отсюда правило: **бэкапить и хранить
`SECRET_KEY` максимально надёжно и отдельно от БД.**
