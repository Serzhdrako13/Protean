...
- В Postgres (в существующем на хосте контейнере, отдельная база/схема
  `protean`) хранится только то, чего нет в conf-файле: пользователи панели,
...
- **Xray (DPI-resistant)**: модульный провайдер для враждебных сетей. Каждый
  **модуль** — один проверенный сквозной стек (transport+security+protocol),
  ...
  между применениями; панель выдаёт client-link. Если модуль не проходит в
  конкретной сети — переключаешь на другой и применяешь заново. Установка
  Xray — из панели (official Xray-install script).
...
Любой из mesh-совместимых провайдеров — **WireGuard, AmneziaWG, OpenVPN,
IKEv2** (все, кроме Xray — у него другая модель, прокси, а не пиринг по
IP) — можно включить в **одну site-to-site сеть** на одном хосте. Клиент на
WireGuard и клиент, например, на IKEv2 видят друг друга и все подсети
площадок как в одной L3-сети — без NAT, адреса сохраняются. Как это
устроено:

- У каждого провайдера свой туннельный CIDR (напр. wg0=10.10.0.0/24,
  awg0=10.20.0.0/24, IKEv2=10.9.0.0/24). Подсети площадок — **общий
  mesh-каталог** (не привязаны к конкретному провайдеру).
- В скачиваемый конфиг клиента панель кладёт AllowedIPs = **туннельные сети
  всех провайдеров, включённых в mesh, + все подсети площадок минус его
  собственные**. Поэтому клиент одного провайдера маршрутизирует трафик к
  подсети другого через хаб.
- Форвардинг между провайдерами устроен по-разному: у wg-семейства
  (WireGuard/AmneziaWG) — прямо в конфиге интерфейса, `PostUp/PostDown` с
  `iptables ... FORWARD ACCEPT` (**без MASQUERADE** — это site-to-site, NAT
  не нужен), с рестартом интерфейса при включении; у cert-based провайдеров
  (OpenVPN/IKEv2) — отдельным host-level `iptables`-правилом, применяется
  сразу, без рестарта. В обоих случаях включается `ip_forward`.
...
- Вкладка **«Обзор сети»** (на странице **«Клиенты»**) показывает все
  транспорты, туннельные CIDR, подсети, всех пиров всех mesh-совместимых
  провайдеров и предупреждает о пересечениях; там же кнопка включения
  форвардинга.
...
- **Дашборд** по каждому провайдеру: интерфейс up/down, endpoint, порт,
  ...
  Автообновление без перезагрузки страницы, интервал настраивается
  (10с/20с/30с/60с/5м, по умолчанию 60с).
...
- **Удаление пира**.
- **Мульти-инстанс**: несколько интерфейсов одного типа на одном сервере
  (wg0, wg1, …) — добавляются на странице **«Узлы VPN»** кнопкой «Добавить
  провайдер», без рестарта контейнера; каждый — отдельный дашборд/инстанс с
  независимыми mesh/egress-настройками (для multisite). Есть и старый
  env-var путь (`WG_INTERFACES`/`AWG_INTERFACES`) для одно-серверных
  инсталляций без UI — см. docs/OPERATIONS.md.
- **Срок действия пира**: авто-disable (wg-семейство) / удаление (cert-based)
  по истечении N дней — временный/гостевой доступ (фоновый воркер).
- **Client-side keygen** (wg-семейство): вставить свой публичный ключ —
  приватный не попадает на сервер (self-managed, без выгрузки конфига).
- **Настройки сервера**: порт, DNS, MTU, для AmneziaWG — ещё и параметры
  обфускации; адрес/подсеть интерфейса задаётся один раз при создании и
  дальше не редактируется (см. «Известные ограничения» ниже про смену
  маски). Изменение требует рестарта интерфейса (панель предупреждает).
- **История конфига** (wg-семейство): перед каждой записью в conf-файл
  панель сохраняет снимок предыдущей версии (дата, размер, превью) —
  восстановить снимок можно прямо из UI, без ручного grep по бэкапам.
...
- **Обзор сети**: вкладка на странице «Клиенты», сводка по всей сети
  (транспорты, туннельные CIDR, подсети, пиры всех mesh-совместимых
  провайдеров, предупреждения о пересечениях, включение форвардинга).
...
## Протестированные дистрибутивы и системные требования

Поддерживаются только systemd-дистрибутивы Linux. Тестирование — через
реальный Docker-контейнер на каждый дистрибутив (настоящий systemd в
качестве PID 1, настоящее SSH-провижининг, реальная логика установки из
`protean-installer.sh`, а не отдельный вручную составленный список
пакетов); по одной версии на семейство пакетного менеджера проверено
локально, все версии ниже прогоняются в CI (по ночам и по запросу):

| Семейство | Версии |
|---|---|
| apt | Ubuntu 22.04, 24.04, 26.04 (18.04/20.04 — best-effort, устарели/сняты с поддержки), Debian 11, 12, 13 |
...
Требования к ресурсам зависят от **протокола VPN**, **числа одновременных
клиентов** и **трафика на клиента**. Измерено реальным нагрузочным тестом
(настоящие туннели и трафик, не синтетика):

| Протокол | Стоимость 1 Мбит/с трафика |
|---|---|
| WireGuard / AmneziaWG | самая дешёвая (шифрование в ядре) |
| IKEv2 | примерно как WireGuard |
| Xray (VLESS+TLS) | ~в 6 раз дороже WireGuard |
| OpenVPN | ~в 21 раз дороже WireGuard (шифрование в одном userspace-процессе) |

Конкретно: 20 одновременных клиентов по ~5 Мбит/с почти не заметны на
WireGuard/IKEv2/Xray (сильно меньше 1 vCPU), а на OpenVPN под них нужно
около 2 vCPU. При 100 клиентах по 10 Мбит/с WireGuard/IKEv2 всё ещё
укладываются в 1-2 vCPU, а одному OpenVPN на тот же трафик нужно порядка
8 vCPU. Если совместимость с клиентами позволяет выбирать — WireGuard/
AmneziaWG или IKEv2 заметно дешевле для большого числа клиентов.

| Тир | vCPU | RAM | Диск | Когда достаточно |
|---|---|---|---|---|
| Минимум | 1 | 1 ГБ | 10 ГБ | Панель + БД, один протокол, немного клиентов на лёгком трафике. Не для OpenVPN как основного протокола. |
| Рекомендуемый | 2 | 2 ГБ | 20 ГБ | 1-2 протокола, десятки клиентов на обычном трафике, есть запас и на OpenVPN. |
| Тяжёлый (OpenVPN как основной, или 100+ клиентов) | 4-8+ | 4 ГБ+ | 20-40 ГБ | Считать CPU по формуле из документа ниже, исходя из своего реального числа клиентов и трафика на клиента. |

RAM и диск почти никогда не узкое место ни для одного из этих
протоколов — упираетесь всегда в CPU. Полные измеренные цифры, рабочая
формула расчёта и честные оговорки о том, что именно гарантируют (а что
нет) цифры «измерено на этом стенде» применительно к реальному
облачному железу — в
**[docs/SYSTEM-REQUIREMENTS.md](docs/SYSTEM-REQUIREMENTS.md)** (на
английском).
...
- [docs/SYSTEM-REQUIREMENTS.md](docs/SYSTEM-REQUIREMENTS.md) — требования к железу, протестированные дистрибутивы, методология нагрузочного теста (на английском)
...
- **Адрес/маска wg-family интерфейса задаётся только при создании
  провайдера** и не редактируется позже — смена маски уже выданным пирам
  могла бы оставить часть из них вне нового диапазона, поэтому поле
  зафиксировано намеренно (и на уровне API тоже, не только в форме).
