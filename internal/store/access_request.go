...
// creates wgpanel.access_request.
...
func (s *Store) UpsertRequest(ctx context.Context, userID int64, provider string) (AccessRequest, error) {
	...
	err := s.pool.QueryRow(ctx, `
		INSERT INTO wgpanel.access_request (user_id, provider, status, updated_at)
...
func (s *Store) ListRequestsForUser(ctx context.Context, userID int64) ([]AccessRequest, error) {
	rows, err := s.pool.Query(ctx, `
		...
		FROM wgpanel.access_request WHERE user_id = $1
	`, userID)
	if err != nil {
		return nil, err
	}
...
func (s *Store) ListAllRequests(ctx context.Context) ([]AccessRequest, error) {
	rows, err := s.pool.Query(ctx, `
		...
		FROM wgpanel.access_request ar
		JOIN wgpanel.users u ON u.id = ar.user_id
...
func (s *Store) DeleteRequest(ctx context.Context, id int64) error {
	tag, err := s.pool.Exec(ctx, `DELETE FROM wgpanel.access_request WHERE id = $1`, id)
	if err != nil {
		return err
	}
...
func (s *Store) SetRequestStatus(ctx context.Context, id int64, status string) error {
	tag, err := s.pool.Exec(ctx, `
		UPDATE wgpanel.access_request SET status = $2, updated_at = now() WHERE id = $1
	`, id, status)
	if err != nil {
		return err
	}
