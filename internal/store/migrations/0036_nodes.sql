...
CREATE TABLE protean.node_peer (
    provider   TEXT        NOT NULL,
    peer_key   TEXT        NOT NULL,
    node_id    BIGINT      NOT NULL REFERENCES protean.nodes(id) ON DELETE CASCADE,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    PRIMARY KEY (provider, peer_key)
);

CREATE INDEX node_peer_node_id_idx ON protean.node_peer(node_id);
