...
- **Несколько серверов (multi-server)**: панель управляет несколькими VPS.
  Серверы добавляются на странице **Servers** (SSH-реквизиты шифруются в БД,
...
- Страница **Mesh** показывает все транспорты, туннельные CIDR, подсети, всех
  пиров обоих провайдеров и предупреждает о пересечениях; там же кнопка
  включения форвардинга (с рестартом интерфейса).
...
- **Mesh**: сводная страница всей сети (транспорты, туннельные CIDR, подсети,
  пиры обоих провайдеров, предупреждения о пересечениях, включение форвардинга).
...
на странице **Servers** — панель подключится к нему по SSH и сама сможет
